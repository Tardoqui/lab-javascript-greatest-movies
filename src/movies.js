...
function getAllDirectors(movies) {
  ...
  const mappedArray = movies.map(function(currentDirector){
...
function howManyMovies(movies) {
  ...
  const spielbergMovies = movies.filter( function (movies){
    return movies.director === 'Steven Spielberg';
  });
  const dramaMovies = spielbergMovies.filter( function (drama){
    return drama.genre.includes('Drama');
  });
  const numberDramaMovies = dramaMovies.length ;
  
  return numberDramaMovies;
}
...
function scoresAverage(array) {   
  ...
  const arrayScores = array.map( function (currentScores){
    return currentScores.score;
  });
  let total = arrayScores.reduce(function(total, numero){
    return total + numero;
  }, 0);
...
function dramaMoviesScore(array) {
  ...
  const dramaMovies = array.filter((currentDrama) => {
    return  (currentDrama.genre.includes('Drama') && currentDrama.score > 0);

});
  console.log(dramaMovies);
  const dramaScore = dramaMovies.map ( function (currentScore) {
    return currentScore.score; 
  });

    let total = dramaScore.reduce( function(total, number){
      return total + number;
    }, 0);
...
console.log(dramaMoviesScore(movies1));
